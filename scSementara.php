...
echo "2. (ERROR, PROSESS) AUTO KOMENTAR + AUTO GET USERSIG + AUTO BANNED FILTER KATA-KATA" . PHP_EOL;
echo "3. GET KOMENTAR + AUTO BANNED FILTER KATA-KATA" . PHP_EOL;
echo "4. AUTO PIN PRODUK / *SOON AUTO PIN BY REQUEST" . PHP_EOL;
echo "5. (ERROR, PROSESS) BALES KOMENTAR / PIN KOMENTAR" . PHP_EOL;
echo "6. AUTO SHOW VOUCHER SEIAP 1 MENIT" . PHP_EOL;
echo "7. (ERROR, PROSESS) AUTO KOMENTAR" . PHP_EOL;
...
function getData()
{
    ...
    $getSession = api("https://api-shopee.mas.mba/?key=$key&sessionid=$sessionId&cookies=" . urlencode($cookies));
...
function getRMTP()
{
    ...
    $getRMTP = api("https://api-shopee.mas.mba/rmtp.php?key=$key&sessionid=$sessionId&cookies=" . urlencode($cookies));
...
//bot komen
function getDUsersig()
{
    global $cookies, $key, $sessionId, $deviceIdKomen, $usersigKomen;
    $getSession = api("https://api-shopee.mas.mba/?key=$key&sessionid=$sessionId&cookies=" . urlencode($cookies));
    $sessionData = json_decode($getSession, true);
    if (!$sessionData) {
        echo $getSession;
    ...
    if ($errCode === 0 && $data && isset($data["session"])) {
        $session = $data["session"];
        $deviceIdKomen = $session["device_id"] ?? '-';
        $usersigKomen = $data['usersig'] ?? '-';
    }
}
function komenLive($message)
{
    getDUsersig();
    global $sessionId, $cookies, $deviceIdKomen, $responseKomen, $usersigKomen;
    ...
    $postData = [
        'uuid' => $deviceIdKomen,
        'usersig' => $usersigKomen,
        'content' => json_encode(['type' => 101, 'content' => $message]),
        'pin' => false,
    ];
